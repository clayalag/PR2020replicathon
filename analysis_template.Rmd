...
drugCorrs <- summarizedPharmacoData %>% 
    group_by(drug) %>%
    summarize(Pearson_ic50  = cor(-log10(auc_GDSC / 10^6), -log10(ic50_CCLE / 10^6), method = "pearson"), Spearman_ic50 = cor(-log10(ic50_GDSC / 10^6), -log10(ic50_CCLE / 10^6), method = "spearman"))
...
drugCorrs <- summarizedData %>% group_by(drug) %>% summarize(Pearson_ic50 = cor(-log10(ic50_GDSC / 10^6), -log10(ic50_CCLE / 10^6), method = "pearson"), Spearman_ic50 = cor(-log10(ic50_GDSC / 10^6), -log10(ic50_CCLE / 10^6), method = "spearman")) #prepping the correlation coefficient data for graph drugCorrs drugCorrs <- gather(drugCorrs, measure, correlation, -drug) drugCorrs

drugCorrs %>% ggplot(aes(x = drug, y = correlation, fill = measure, group = measure)) + geom_bar(stat = "identity", position = position_dodge(), colour = "black") + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + scale_fill_grey() + ylim(0, 1) + ggtitle("Correlation of cell line IC50 summaries between studies for each drug")
```
