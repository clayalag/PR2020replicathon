...
drugCorrs
drugCorrs <- tidyr::gather(drugCorrs, measure, correlation, -drug)
...
drugCorrs
#x <- rnorm(50)
#perfect <- data.frame(x=, y=x)
Pearson_auc  = cor((summarizedPharmacoData$auc_GDSC), (summarizedPharmacoData$auc_CCLE), method = "pearson")
Spearman_auc = cor(-log10(summarizedPharmacoData$auc_GDSC), -log10(summarizedPharmacoData$auc_CCLE), method = "spearman")
#Pearson_AUC_GDSC <- cor((summarizedPharmacoData$auc_GDSC), method = "pearson")
#Pearson_AUC_CCLE <- cor((summarizedPharmacoData$auc_CCLE), method = "pearson")
cor.coef <- round(cor((summarizedPharmacoData$auc_GDSC), (summarizedPharmacoData$auc_CCLE), method = "pearson"))
ggplot(data=drugCorrs, aes(x=Pearson_auc,y=Spearman_auc)) +
  geom_point() +
...
drugCorrs %>%
    ggplot(aes(x = Pearson_auc, y = Spearman_auc, label = drug)) +
